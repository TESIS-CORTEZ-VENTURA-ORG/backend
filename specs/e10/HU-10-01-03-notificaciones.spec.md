...
- **HU-10-04** (alertas accionables de IA) → requiere el **servicio de IA (E08, FastAPI)**. El campo `data Json?` ya admite el deep-link / payload accionable (`action button`) que la HU pide; cuando E08 exista, creará notificaciones `type='system'` (u otro) con su `data`. **Cerrada en el Lote B1** (E08 ya en producción) — ver `HU-10-04-forecast-shortfall.spec.md`: nuevo `type='forecast_shortfall'`, dirigida a owner/manager vía `NotificationsService.createForRolesTx`.
...
- `notificationTypeSchema` = `z.enum(['low_stock','order_ready','bill_requested','system','forecast_shortfall'])` (`forecast_shortfall` agregado en el Lote B1 — ver `HU-10-04-forecast-shortfall.spec.md`).
