...
**Tests:** `src/chat/sql-validator.util.spec.ts` (validator, 40+ unit tests cubriendo los 9 vectores de ataque) + `src/chat/chat.service.spec.ts` (mapeo de errores de ejecución 502/504, nuevo) + `src/chat/schema-context.spec.ts` (guarda de regresión columna-por-columna, nuevo) + `test/chat.e2e-spec.ts` (happy path, RBAC staff 403/401, 10 security vectors, RLS cross-tenant isolation, degradación de ejecución 502, low-stock contra `ingredients.stock/min_stock` reales).
